...
function solve(){
	return function (animals) {
		...
		.groupBy('species')
		.sortBy(function(grp) {
			return grp[0].species;
		})
		.reverse()
		.value();

		_.each(groupedAnimals, function(grp) {
			var separator = new Array(grp[0].species.length+2).join('-');
			console.log(separator);
			console.log(grp[0].species + ':');
			console.log(separator);

			_.each(grp, function(animal) {
